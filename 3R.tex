...
\title{AI \(\Rightarrow\) RI, 3R Scientific Modelling}
\author{Ed Darnell}
\date{\today}
...
\maketitle
\begin{abstract}
    This document clearly defines logic, \(\qbit\) and 3R scientific modelling. Our brains, while proficient in crafting 3D models of the macroscopic world, are inherently limited. In our daily lives, we often navigate complex issues using oversimplified categories—right or wrong, true or false, 1 or 0—without a detailed understanding of how logic is defined. 3R requires a deeper, analytic understanding of number, complexity and \(\qbit\) uncertainty.
...
In logic, \textbf{true} and \textbf{false} are defined within a logical framework. These are not inherent properties of reality but are assigned based on the rules and definitions of the system. Binary logic, which defines \texttt{0} as \texttt{NOT 1}, is foundational. This approach avoids ambiguity by providing a clear, singular base definition. Logic is best viewed as a language, the language of mathematics and computing. In contrast to historic languages it is constructed in a way which avoids any ambiguity or inconsistency.

Reality itself is continuous and consistent. It does not possess a "false" form. Rather, it is a singular entity that exists independently of descriptions, logic and models. Reality is best modelled as operating in a continuous but relativistic manner, meaning it cannot be fully captured by discrete binary states. Binary logic forms the best possible language to model reality. Words are neural, pre-logic language and should be viewed as ambiguous until logically defined within a model.

Logical models can both over-simplify and over-complicate. Our models must recognize the superposition of causality and probability. For example, with 100 random coin tosses, modelled as \( X \sim B(100,0.5) \), our model predicts a 98.24 percent chance of between 60 and 80 heads. The model cannot however predict the outcome of each individual toss, nor can it prevent all 100 coins being placed heads up (impossibly low model probability of less than  \( 8 \times 10^{-29} \)). Our model of coin tossing is a model, a highly logical model, with a high degree of confidence if the modelling requirement of randomness is met, but it is a model of reality, not reality. No real event can be either of the logical extremes of perfectly random or perfectly causal. To progress science must embrace the uncertainty of relativistic and quantum modelling which an understanding of \(\qbit\) and 3R explains. Models are always uncertain, however logic can itemise the assumptions and quantify the uncertainty.
